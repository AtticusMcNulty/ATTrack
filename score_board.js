...
window.onload = function () {
  document
    .getElementById("scoreline_el3")
    .setAttribute(
      "size",
      document.getElementById("scoreline_el3").getAttribute("placeholder")
        .length + 2
    );
...
function run_achievement_tabs() {
  let tabs = document
    .getElementById("achievements_section")
    .querySelectorAll("[data-tab-target-achievement]");
  let tabContents = document
    .getElementById("achievements_section")
    .querySelectorAll("[data-tab-content-achievement]");

  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      let target = document
        .getElementById("achievements_section")
        .querySelector(tab.dataset.tabTargetAchievement);
...
document
  .querySelector("#analytics_chart_dropdown")
  .addEventListener("change", function (event) {
    let maximum = 1;

    let recordOfGames = JSON.parse(
      window.localStorage.getItem("recordOfGames")
    );
...
function update_current_game() {
  ...
  document.getElementById("lastgame_stats_header2").textContent =
    "Match Rating: 6";
  document.getElementById("lastgame_stats_header2").style.fontFamily =
    "poppins";
  ...
  document.getElementById("lastgame_stats_header5").textContent =
    "Key Passes: 0";

  document.getElementById("lastgame_stats_header6").textContent = "Shots: 0";
  document.getElementById("lastgame_stats_header7").textContent =
    "Shots on Target: 0";
  document.getElementById("lastgame_stats_header8").textContent =
    "Shots off Target: 0";
  document.getElementById("lastgame_stats_header9").textContent =
    "Shots that Hit Post: 0";
  document.getElementById("lastgame_stats_header10").textContent =
    "Shots on Target %: 0";
  document.getElementById("lastgame_stats_header11").textContent =
    "Shots per Goal: 0";

  document.getElementById("lastgame_stats_header12").textContent =
    "Passes Attempted: 0";
  document.getElementById("lastgame_stats_header13").textContent =
    "Passes Completed: 0";
  document.getElementById("lastgame_stats_header14").textContent =
    "Passes Missed: 0";
  document.getElementById("lastgame_stats_header15").textContent =
    "Passes Completion %: 0";

  document.getElementById("lastgame_stats_header16").textContent =
    "Dribbles Attempted: 0";
  document.getElementById("lastgame_stats_header17").textContent =
    "Successful Dribbles: 0";
  document.getElementById("lastgame_stats_header18").textContent =
    "Failed Dribbles: 0";
  document.getElementById("lastgame_stats_header19").textContent =
    "Dribble Completion %: 0";
  document.getElementById("lastgame_stats_header20").textContent =
    "Fouls Won: 0";

  document.getElementById("lastgame_stats_header21").textContent = "Tackles: 0";
  document.getElementById("lastgame_stats_header22").textContent =
    "Clearances: 0";
  document.getElementById("lastgame_stats_header23").textContent =
    "Interceptions: 0";

  document.getElementById("lastgame_stats_header24").textContent =
    "Duels Attempted: 0";
  document.getElementById("lastgame_stats_header25").textContent =
    "Duels Won: 0";
  document.getElementById("lastgame_stats_header26").textContent =
    "Duels Lost: 0";
  document.getElementById("lastgame_stats_header27").textContent =
    "Duel Success %: 0";
  document.getElementById("lastgame_stats_header28").textContent =
    "Turnovers: 0";

  document.getElementById("lastgame_stats_header29").textContent = "Fouls: 0";
  document.getElementById("lastgame_stats_header30").textContent =
    "Yellow Cards: 0";
  document.getElementById("lastgame_stats_header31").textContent =
    "Red Card: 0";
}
...
function updateTimer() {
  ...
  document.getElementById("timer_header").textContent =
...
function add_count(btnNum) {
  ...
  let lastGameStatHeader = document.getElementById(
    `lastgame_stats_header${btnNum}`
  );

  if (deleteMode == false) {
    lastgameStats[`lastgameStat${btnNum}`] =
      lastgameStats[`lastgameStat${btnNum}`] + 1;
  } else {
    lastgameStats[`lastgameStat${btnNum}`] =
      lastgameStats[`lastgameStat${btnNum}`] - 1;
  }
...
function update_match_rating() {
  ...
  lastgameStats.lastgameStat2 =
    Number(
      (
        lastgameStats.lastgameStat3 * 0.9 +
        lastgameStats.lastgameStat4 * 0.7 +
        lastgameStats.lastgameStat5 * 0.2 +
        lastgameStats.lastgameStat7 * 0.1 +
        lastgameStats.lastgameStat8 * -0.1 +
        lastgameStats.lastgameStat9 * 0.15 +
        lastgameStats.lastgameStat13 * 0.05 +
        lastgameStats.lastgameStat14 * -0.05 +
        lastgameStats.lastgameStat17 * 0.1 +
        lastgameStats.lastgameStat18 * -0.1 +
        lastgameStats.lastgameStat20 * 0.1 +
        lastgameStats.lastgameStat21 * 0.1 +
        lastgameStats.lastgameStat22 * 0.05 +
        lastgameStats.lastgameStat23 * 0.1 +
        lastgameStats.lastgameStat25 * 0.1 +
        lastgameStats.lastgameStat26 * -0.1 +
        lastgameStats.lastgameStat28 * -0.15 +
        lastgameStats.lastgameStat29 * -0.1 +
        lastgameStats.lastgameStat30 * -0.5 +
        lastgameStats.lastgameStat31 * -1.5
      ).toFixed(2)
    ) + 6;
...
function add_opponent_goal() {
  let opponentScoreHeader = document.getElementById(
    "opponent_score_in_game_header"
  );
...
function update_other_shot_stats() {
  lastgameStats.lastgameStat6 =
    lastgameStats.lastgameStat7 +
    lastgameStats.lastgameStat8 +
    lastgameStats.lastgameStat9;
  document.getElementById(
    "lastgame_stats_header6"
  ).textContent = `Shots: ${lastgameStats.lastgameStat6}`;

  lastgameStats.lastgameStat10 =
    lastgameStats.lastgameStat7 / lastgameStats.lastgameStat6;
  document.getElementById(
    "lastgame_stats_header10"
  ).textContent = `Shots on Target %: ${(
    lastgameStats.lastgameStat10 * 100
  ).toFixed(1)}`;

  lastgameStats.lastgameStat11 =
    lastgameStats.lastgameStat6 / lastgameStats.lastgameStat3;

  if (lastgameStats.lastgameStat3 == 0) {
    document.getElementById(
      "lastgame_stats_header11"
    ).textContent = `Shots per Goal: N/A`;
  } else {
    document.getElementById(
      "lastgame_stats_header11"
    ).textContent = `Shots per Goal: ${lastgameStats.lastgameStat11.toFixed(
      1
    )}`;
  }
}
...
function update_other_pass_stats() {
  lastgameStats.lastgameStat12 =
    lastgameStats.lastgameStat13 + lastgameStats.lastgameStat14;
  document.getElementById(
    "lastgame_stats_header12"
  ).textContent = `Passes Attempted: ${lastgameStats.lastgameStat12}`;

  lastgameStats.lastgameStat15 =
    lastgameStats.lastgameStat13 / lastgameStats.lastgameStat12;
  document.getElementById(
    "lastgame_stats_header15"
  ).textContent = `Passes Completion %: ${(
    lastgameStats.lastgameStat15 * 100
  ).toFixed(1)}`;
}

function update_other_dribble_stats() {
  lastgameStats.lastgameStat16 =
    lastgameStats.lastgameStat17 + lastgameStats.lastgameStat18;
  document.getElementById(
    "lastgame_stats_header16"
  ).textContent = `Dibbles Attempted: ${lastgameStats.lastgameStat16}`;

  lastgameStats.lastgameStat19 =
    lastgameStats.lastgameStat17 / lastgameStats.lastgameStat16;
  document.getElementById(
    "lastgame_stats_header19"
  ).textContent = `Dribble Completion %: ${(
    lastgameStats.lastgameStat19 * 100
  ).toFixed(1)}`;
}

function update_other_duel_stats() {
  lastgameStats.lastgameStat24 =
    lastgameStats.lastgameStat25 + lastgameStats.lastgameStat26;
  document.getElementById(
    "lastgame_stats_header24"
  ).textContent = `Duels Attempted: ${lastgameStats.lastgameStat24}`;

  lastgameStats.lastgameStat27 =
    lastgameStats.lastgameStat25 / lastgameStats.lastgameStat24;
  document.getElementById(
    "lastgame_stats_header27"
  ).textContent = `Duel Success %: ${(
    lastgameStats.lastgameStat27 * 100
  ).toFixed(1)}`;
}
...
function updatePerGameStats() {
  ...
  document.getElementById(
    "games_played_header"
  ).textContent = `Games Played: ${gamesPlayedAT}`;
  ...
  for (let i = 1; i <= 31; i++) {
    ...
    if (i == 1) {
      ...
      perGameStatHeader.textContent = `Shots On Target %: ${perGameStat.toFixed(
        1
      )}`;
    } else if (i == 11) {
      perGameStat = shotsAT / goalsAT;
      perGameStatHeader.textContent = `Shots Per Goal: ${perGameStat.toFixed(
        1
      )}`;
    } else if (i == 12) {
      ...
      perGameStat = allTimeStat;
      perGameStatHeader.textContent = `Pass Completion %: ${perGameStat.toFixed(
        1
      )}`;
    } else if (i == 16) {
      ...
      perGameStat = allTimeStat;
      perGameStatHeader.textContent = `Dribble Completion %: ${perGameStat.toFixed(
        1
      )}`;
    } else if (i == 20) {
      ...
      perGameStat = allTimeStat;
      perGameStatHeader.textContent = `Duels Success %: ${perGameStat.toFixed(
        1
      )}`;
    } else if (i == 28) {
...
function compareStats() {
  for (let i = 1; i <= 31; i++) {
    ...
    if (i == 1 || i == 2) {
      ...
      let lastgameStatFixed = Number(
        (lastgameStats[`lastgameStat${i}`] * 100).toFixed(1)
      );
      let pergameStatFixed = Number(pergameStats[i - 1].toFixed(1));
      ...
      let lastgameStatFixed = Number(
        lastgameStats[`lastgameStat${i}`].toFixed(2)
      );
...
function performance_ranking() {
  ...
  let matchRatingHeader = document.getElementById(
    "feedback_match_rating_header"
  );
  let minutesPlayedHeader = document.getElementById(
    "feedback_minutes_played_header"
  );
  ...
  if (lastgameStats.lastgameStat2 >= 10) {
    ...
    summaryHeader.textContent =
      "Summary: Excellent performance! You made a significant contribution to the team's success.";
    gradeHeader.textContent = "Grade: A";
  } else if (lastgameStats.lastgameStat2 >= 8.5) {
    summaryHeader.textContent =
      "Summary: Great performance! You played an important role in the team's success.";
    gradeHeader.textContent = "Grade: A-";
  } else if (lastgameStats.lastgameStat2 >= 8) {
    summaryHeader.textContent =
      "Summary: Good performance, but there is room for improvement. Keep working hard!";
    gradeHeader.textContent = "Grade: B+";
  } else if (lastgameStats.lastgameStat2 >= 7.5) {
    summaryHeader.textContent =
      "Summary: Solid performance, but there were some areas where you could have done better.";
    gradeHeader.textContent = "Grade: B";
    ...
    summaryHeader.textContent =
      "Summary: Above average performance. Let's focus on improving in training.";
    gradeHeader.textContent = "Grade: C+";
  } else if (lastgameStats.lastgameStat2 >= 5.5) {
    summaryHeader.textContent =
      "Summary: Average performance. We need you to step up and play to your full potential.";
    gradeHeader.textContent = "Grade: C";
  } else if (lastgameStats.lastgameStat2 >= 4.5) {
    summaryHeader.textContent =
      "Summary: Below average performance. We believe in your abilities, but we need to see more from you.";
    gradeHeader.textContent = "Grade: C-";
  } else if (lastgameStats.lastgameStat2 >= 4) {
    summaryHeader.textContent =
      "Summary: Poor performance. Let's work together to identify areas to improve.";
    gradeHeader.textContent = "Grade: D+";
  } else if (lastgameStats.lastgameStat2 >= 3.5) {
    summaryHeader.textContent =
      "Summary: Terrible performance. We know you can do better than this, let's work on it together.";
    gradeHeader.textContent = "Grade: D";
  } else if (lastgameStats.lastgameStat2 >= 3) {
    summaryHeader.textContent =
      "Summary: Unacceptable performance. We need to see significant improvements in your play.";
    gradeHeader.textContent = "Grade: D-";
  } else {
    summaryHeader.textContent =
      "Summary: Abysmal performance. You need to step up your game significantly.";
  ...
  matchRatingHeader.textContent = `Match Rating: ${lastgameStats.lastgameStat2.toFixed(
    2
  )}`;
  matchRatingHeader.style.fontFamily = "poppins";
  matchRatingHeader.style.fontWeight = "normal";
...
function set_advice(adviceHeader, num) {
  if (lastgameStats.lastgameStat6 < 2 && !adviceSelected.includes(1)) {
    ...
    lastgameStats.lastgameStat4 == 0 &&
    lastgameStats.lastgameStat5 == 0 &&
    !adviceSelected.includes(2)
  ) {
    ...
    lastgameStats.lastgameStat10 < 0.33 &&
    !adviceSelected.includes(3)
  ) {
    adviceHeader.textContent = `Advice #${num}: Only ${(
      lastgameStats.lastgameStat10 * 100
    ).toFixed(
      2
    )}% your shots were on target this game. Take a moment to analyze your shooting technique and approach, practice your accuracy through drills and repetition, and maintain a confident and composed mindset when in front of the goal.`;
    ...
    lastgameStats.lastgameStat15 < 0.6 &&
    !adviceSelected.includes(5)
  ) {
    adviceHeader.textContent = `Advice #${num}: You completed only ${(
      lastgameStats.lastgameStat15 * 100
    ).toFixed(
      2
    ...
    lastgameStats.lastgameStat19 < 0.4 &&
    !adviceSelected.includes(7)
  ) {
    adviceHeader.textContent = `Advice #${num}: With a dribble completion rate of ${(
      lastgameStats.lastgameStat19 * 100
    ).toFixed(
      2
    ...
    lastgameStats.lastgameStat25 < lastgameStats.lastgameStat26 &&
    !adviceSelected.includes(9)
  ) {
    adviceHeader.textContent = `Advice #${num}: You lost ${
      lastgameStats.lastgameStat26 - lastgameStats.lastgameStat25
    ...
    lastgameStats.lastgameStat31 == 1 &&
    !adviceSelected.includes(11)
  ) {
...
function initialize_all_time_stats() {
  ...
  if (!redeemedArr)
    window.localStorage.setItem("redeemedArr", JSON.stringify([]));
  ...
  if (winsAT && lossesAT && tiesAT) {
    document.getElementById(
      "record_at_header"
    ).textContent = `Record: ${winsAT}W - ${lossesAT}L - ${tiesAT}T`;
  } else {
  ...
  let passCompletionPercentageAT =
    window.localStorage.getItem("alltime_stat15");
  ...
  let dribbleCompletionPercentageAT =
    window.localStorage.getItem("alltime_stat19");
  ...
  let alltimeStats = [
    ...
    shotsOffTargetAT,
    shotsHitPostAT,
    shotsOnTargetPercentageAT,
    shotsPerGoalAT,
    passesAttemptedAT,
    passesCompletedAT,
    passesMissedAT,
    passCompletionPercentageAT,
    dribblesAttemptedAT,
    successfulDribblesAT,
    failedDribblesAT,
    dribbleCompletionPercentageAT,
    foulsWonAT,
    tacklesAT,
    clearancesAT,
    interceptionsAT,
  ...
  if (!recordOfGames)
    window.localStorage.setItem("recordOfGames", JSON.stringify([]));
...
function allTimeStats(amounts) {
  ...
  let shots =
    Number(window.localStorage.getItem("alltime_stat6")) +
    amounts[6] +
    amounts[7] +
    amounts[8];
  document.getElementById(
    "alltime_stats_header6"
  ).textContent = `Shots: ${shots}`;
  window.localStorage.setItem("alltime_stat6", shots);

  let passes =
    Number(window.localStorage.getItem("alltime_stat12")) +
    amounts[12] +
    amounts[13];
  document.getElementById(
    "alltime_stats_header12"
  ).textContent = `Passes: ${passes}`;
  window.localStorage.setItem("alltime_stat12", passes);

  let dribbles =
    Number(window.localStorage.getItem("alltime_stat16")) +
    amounts[16] +
    amounts[17];
  document.getElementById(
    "alltime_stats_header16"
  ).textContent = `Dribbles: ${dribbles}`;
  window.localStorage.setItem("alltime_stat16", dribbles);

  let duels =
    Number(window.localStorage.getItem("alltime_stat24")) +
    amounts[24] +
    amounts[25];
  document.getElementById(
    "alltime_stats_header24"
  ).textContent = `Duels: ${duels}`;
  window.localStorage.setItem("alltime_stat24", duels);

  for (let i = 1; i <= 31; i++) {
    let alltimeStatHeader = document.getElementById(`alltime_stats_header${i}`);
    let highscoreStatHeader = document.getElementById(
      `highscore_stats_header${i}`
    );

    let alltimeStat = Number(window.localStorage.getItem(`alltime_stat${i}`));
    let highscoreStat = Number(
      window.localStorage.getItem(`highscore_stat${i}`)
    );

    // add amount if i is not a total stat (total stats have already been calculated)
    if (i != 6 && i != 12 && i != 16 && i != 24) alltimeStat += amounts[i - 1];

    if (i == 1 || i == 2) alltimeStat = alltimeStat.toFixed(1);
    ...
    if (i == 1) {
      ...
      highscoreStat = parseFloat(
        window.localStorage.getItem(`highscore_stat${i}`)
      );

      let shotsAT = Number(window.localStorage.getItem("alltime_stat6"));
      let shotsOnTargetAT = Number(
        window.localStorage.getItem("alltime_stat7")
      );
      ...
      let passesCompletedAT = Number(
        window.localStorage.getItem("alltime_stat13")
      );
      let passesAttemptedAT = Number(
        window.localStorage.getItem("alltime_stat12")
      );
      ...
      if (amounts[i - 1] == -1) {
        ...
        alltimeStat = ((passesCompletedAT / passesAttemptedAT) * 100).toFixed(
          1
        );
      }
      ...
      let dribblesCompletedAT = Number(
        window.localStorage.getItem("alltime_stat17")
      );
      let dribblesAttemptedAT = Number(
        window.localStorage.getItem("alltime_stat16")
      );
      ...
      if (amounts[i - 1] == -1) {
        ...
        alltimeStat = (
          (dribblesCompletedAT / dribblesAttemptedAT) *
          100
        ).toFixed(1);
      }
      ...
      let duelsAttemptedAT = Number(
        window.localStorage.getItem("alltime_stat24")
      );
...
function initialize_highscore_stats() {
  ...
  let shotsOnTargetPercentageHighscore =
    window.localStorage.getItem("highscore_stat10");
  let shotsPerGoalHighscore = window.localStorage.getItem("highscore_stat11");

  let passesAttemptedHighscore =
    window.localStorage.getItem("highscore_stat12");
  let passesCompletedHighscore =
    window.localStorage.getItem("highscore_stat13");
  let passesMissedHighscore = window.localStorage.getItem("highscore_stat14");
  let passCompletionPercentageHighscore =
    window.localStorage.getItem("highscore_stat15");

  let dribblesAttemptedHighscore =
    window.localStorage.getItem("highscore_stat16");
  let successfulDribblesHighscore =
    window.localStorage.getItem("highscore_stat17");
  let failedDribblesHighscore = window.localStorage.getItem("highscore_stat18");
  let dribbleCompletionPercentageHighscore =
    window.localStorage.getItem("highscore_stat19");
  ...
  let duelSuccessPercentageHighscore =
    window.localStorage.getItem("highscore_stat27");
  ...
  let highscoreStats = [
    ...
    keyPassesHighscore,
    shotsHighscore,
    shotsOnTargetHighscore,
    shotsOffTargetHighscore,
    shotsHitPostHighscore,
    shotsOnTargetPercentageHighscore,
    shotsPerGoalHighscore,
    passesAttemptedHighscore,
    passesCompletedHighscore,
    passesMissedHighscore,
    passCompletionPercentageHighscore,
    dribblesAttemptedHighscore,
    successfulDribblesHighscore,
    failedDribblesHighscore,
    dribbleCompletionPercentageHighscore,
    foulsWonHighscore,
    tacklesHighscore,
    clearancesHighscore,
    interceptionsHighscore,
    duelsAttemptedHighscore,
    duelsWonHighscore,
    duelsLostHighscore,
    duelSuccessPercentageHighscore,
    turnoversHighscore,
    foulsHighscore,
    yellowCardsHighscore,
    redCardsHighscore,
  ];
  ...
  for (let i = 1; i <= 31; i++) {
    if (highscoreStats[i - 1]) {
      ...
      if (i == 1) {
        ...
        statHeader.textContent = `Acc. Match Rating: ${Number(
          matchRatingHighscore
        ).toFixed(1)}`;
      } else if (i == 3) {
...
function update_last_match_stats() {
  document.getElementById(
    "lastgame_stats_header1"
  ).textContent = `Minutes: ${lastgameStats.lastgameStat1}`;
  document.getElementById(
    "lastgame_stats_header2"
  ).textContent = `Match Rating: ${lastgameStats.lastgameStat2.toFixed(1)}`;
  document.getElementById("lastgame_stats_header2").style.fontFamily =
    "poppins";
  document.getElementById("lastgame_stats_header2").style.fontWeight = "normal";

  document.getElementById(
    "lastgame_stats_header3"
  ).textContent = `Goals: ${lastgameStats.lastgameStat3}`;
  document.getElementById(
    "lastgame_stats_header4"
  ).textContent = `Assists: ${lastgameStats.lastgameStat4}`;
  document.getElementById(
    "lastgame_stats_header5"
  ).textContent = `Key Passes: ${lastgameStats.lastgameStat5}`;

  document.getElementById(
    "lastgame_stats_header6"
  ).textContent = `Shots: ${lastgameStats.lastgameStat6}`;
  document.getElementById(
    "lastgame_stats_header7"
  ).textContent = `Shots on Target: ${lastgameStats.lastgameStat7}`;
  document.getElementById(
    "lastgame_stats_header8"
  ).textContent = `Shots off Target: ${lastgameStats.lastgameStat8}`;
  document.getElementById(
    "lastgame_stats_header9"
  ).textContent = `Shots that Hit Post: ${lastgameStats.lastgameStat9}`;
  document.getElementById(
    "lastgame_stats_header10"
  ).textContent = `Shots on Target %: ${(
    lastgameStats.lastgameStat10 * 100
  ...
  document.getElementById(
    "lastgame_stats_header11"
  ).textContent = `Shots per Goal: ${(
    lastgameStats.lastgameStat11 * 100
  ).toFixed(1)}`;
  ...
  document.getElementById(
    "lastgame_stats_header13"
  ).textContent = `Passes Completed: ${lastgameStats.lastgameStat13}`;
  document.getElementById(
    "lastgame_stats_header14"
  ).textContent = `Passes Missed: ${lastgameStats.lastgameStat14}`;
  document.getElementById(
    "lastgame_stats_header15"
  ).textContent = `Pass Completion %: ${(
    lastgameStats.lastgameStat15 * 100
  ).toFixed(1)}`;

  document.getElementById(
    "lastgame_stats_header16"
  ).textContent = `Dribbles Attempted: ${lastgameStats.lastgameStat16}`;
  document.getElementById(
    "lastgame_stats_header17"
  ).textContent = `Successful Dribbles: ${lastgameStats.lastgameStat17}`;
  document.getElementById(
    "lastgame_stats_header18"
  ).textContent = `Failed Dribbles: ${lastgameStats.lastgameStat18}`;
  document.getElementById(
    "lastgame_stats_header19"
  ).textContent = `Dribble Completion %: ${(
    lastgameStats.lastgameStat19 * 100
  ...
  document.getElementById(
    "lastgame_stats_header20"
  ).textContent = `Fouls Won: ${lastgameStats.lastgameStat20}`;

  document.getElementById(
    "lastgame_stats_header21"
  ).textContent = `Tackles: ${lastgameStats.lastgameStat21}`;
  document.getElementById(
    "lastgame_stats_header22"
  ).textContent = `Clearances: ${lastgameStats.lastgameStat22}`;
  document.getElementById(
    "lastgame_stats_header23"
  ).textContent = `Interceptions: ${lastgameStats.lastgameStat23}`;

  document.getElementById(
    "lastgame_stats_header24"
  ).textContent = `Duels Attempted: ${lastgameStats.lastgameStat24}`;
  document.getElementById(
    "lastgame_stats_header25"
  ).textContent = `Duels Won: ${lastgameStats.lastgameStat25}`;
  document.getElementById(
    "lastgame_stats_header26"
  ).textContent = `Duels Lost: ${lastgameStats.lastgameStat26}`;
  document.getElementById(
    "lastgame_stats_header27"
  ).textContent = `Duel Success %: ${(
    lastgameStats.lastgameStat27 * 100
  ...
  document.getElementById(
    "lastgame_stats_header28"
  ).textContent = `Turnovers: ${lastgameStats.lastgameStat28}`;

  document.getElementById(
    "lastgame_stats_header29"
  ).textContent = `Fouls: ${lastgameStats.lastgameStat29}`;
  document.getElementById(
    "lastgame_stats_header30"
  ).textContent = `Yellow Cards: ${lastgameStats.lastgameStat30}`;
  document.getElementById(
    "lastgame_stats_header31"
  ).textContent = `Red Card: ${lastgameStats.lastgameStat31}`;
}
...
function reset_in_game_stats() {
  ...
  document.getElementById(
    "scoreline_el1"
  ).textContent = `OL Reign ${teamGoals} - `;
  ...
  document.getElementById(
    "ingame_stats_btn7"
  ).textContent = `On Target: ${lastgameStats.lastgameStat7}`;
  document.getElementById(
    "ingame_stats_btn8"
  ).textContent = `Off Target: ${lastgameStats.lastgameStat8}`;
  document.getElementById(
    "ingame_stats_btn9"
  ).textContent = `Hit Post: ${lastgameStats.lastgameStat9}`;

  document.getElementById(
    "ingame_stats_btn13"
  ).textContent = `Completed: ${lastgameStats.lastgameStat13}`;
  document.getElementById(
    "ingame_stats_btn14"
  ).textContent = `Missed: ${lastgameStats.lastgameStat14}`;

  document.getElementById(
    "ingame_stats_btn17"
  ).textContent = `Successful: ${lastgameStats.lastgameStat17}`;
  document.getElementById(
    "ingame_stats_btn18"
  ).textContent = `Failed: ${lastgameStats.lastgameStat18}`;
  document.getElementById(
    "ingame_stats_btn20"
  ).textContent = `Fouls Won: ${lastgameStats.lastgameStat20}`;
  ...
  document.getElementById(
    "ingame_stats_btn31"
  ).textContent = `Red Cards: ${lastgameStats.lastgameStat31}`;
}
...
function create_log_game() {
  ...
  matchRatingLogElement.textContent = `MR: ${lastgameStats.lastgameStat2.toFixed(
    2
  )} `;
  ...
  if (historySection.firstChild.nextSibling.nextSibling) {
    historySection.insertBefore(
      newGameLogDiv,
      historySection.firstChild.nextSibling.nextSibling
    );
  } else {
...
function deleteGameLog(logCount) {
  ...
  let divminrapper = document.querySelector(`#deleteGameLogBtn${logCount}`)
...
let totalPoints = 0;
let thresholds = [
  0, 150, 300, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 2750, 3000,
  3250, 3500, 3750, 4000,
];
let thresholdTitles = [
  "Rookie",
...
function run_achievements() {
  ...
  let shotsOnTargetHighscore = Number(
    window.localStorage.getItem("highscore_stat7")
  );
  let shotsOnTargetAT = Number(window.localStorage.getItem("alltime_stat7"));

  let shotsOnTargetPercentageHighscore = Number(
    window.localStorage.getItem("highscore_stat10")
  );

  let shotsHitPostHighscore = Number(
    window.localStorage.getItem("highscore_stat9")
  );
  let shotsHitPostAT = Number(window.localStorage.getItem("alltime_stat9"));

  let shotsOffTargetHighscore = Number(
    window.localStorage.getItem("highscore_stat8")
  );
  let shotsOffTargetAT = Number(window.localStorage.getItem("alltime_stat8"));

  let passesCompletedHighscore = Number(
    window.localStorage.getItem("highscore_stat13")
  );
  let passesCompletedAT = Number(window.localStorage.getItem("alltime_stat13"));

  let passesAttempedHighscore = Number(
    window.localStorage.getItem("highscore_stat12")
  );
  let passesAttempedAT = Number(window.localStorage.getItem("alltime_stat12"));

  let passesMissedHighscore = Number(
    window.localStorage.getItem("highscore_stat14")
  );
  let passesMissedAT = Number(window.localStorage.getItem("alltime_stat14"));

  let successfulDribblesHighscore = Number(
    window.localStorage.getItem("highscore_stat17")
  );
  let successfulDribblesAT = Number(
    window.localStorage.getItem("alltime_stat17")
  );

  let dribblesAttempedHighscore = Number(
    window.localStorage.getItem("highscore_stat16")
  );
  let dribblesAttempedAT = Number(
    window.localStorage.getItem("alltime_stat16")
  );

  let failedDribblesHighscore = Number(
    window.localStorage.getItem("highscore_stat18")
  );
...
function run_achievement_highscore(stat, amount) {
  ...
  let innerProgressBar = document.getElementById(
    `achievement_${achievementNumber}_progressbar_inner`
  );
  let outerProgressBar = document.getElementById(
    `achievement_${achievementNumber}_progressbar_outer`
  );
  let achievement = document.getElementById(`achievement_${achievementNumber}`);
  let points = document.getElementById(
    `achievement_${achievementNumber}_points`
  );
...
function reedem_prize(prizeNumber) {
  ...
  let prizeCost = Number(
    document.getElementById(`prize_${prizeNumber}_cost`).getAttribute("value")
  );

  let pointsRedeemedAT = Number(
    window.localStorage.getItem("pointsRedeemedAT")
  );
...
function add_mr_to_points() {
  let matchRatingPointsAT = Number(
    window.localStorage.getItem("matchRatingPointsAT")
  );

  totalPoints =
    totalPoints + (Math.round(lastgameStats.lastgameStat2) - 6) * 20;
  matchRatingPointsAT =
    matchRatingPointsAT + (Math.round(lastgameStats.lastgameStat2) - 6) * 20;

  Swal.fire({
    title: `You scored a match rating of ${lastgameStats.lastgameStat2.toFixed(
      2
    )}`,
    text: `This has awarded you ${
      (Math.round(lastgameStats.lastgameStat2) - 6) * 20
    } points`,
...
function initialize_clothing_arr() {
  ...
  if (!default_arr) {
    window.localStorage.setItem(
      "default_arr",
      JSON.stringify(["jersey", "shorts", "shoes", "badge"])
    );
    run = false;
  }
  if (!argentina_arr) {
  ...
  clothing_arr = [
    JSON.parse(default_arr),
    JSON.parse(argentina_arr),
    JSON.parse(brazil_arr),
    JSON.parse(france_arr),
  ];
...
function initialize_selectors() {
  ...
  if (!selector_arr) {
    window.localStorage.setItem(
      "selector_arr",
      JSON.stringify(["default", "default", "default", "default"])
    );
  } else {
    ...
    document.querySelector(
      `#${jerseyID}_image`
    ).src = `images/${jerseyName}_${jerseyID}.png`;
    document.querySelector(
      `#${shortsID}_image`
    ).src = `images/${shortsName}_${shortsID}.png`;
    document.querySelector(
      `#${shoesID}_image`
    ).src = `images/${shoesName}_${shoesID}.png`;
    document.querySelector(
      `#${badgeID}_image`
    ).src = `images/${badgeName}_${badgeID}.png`;
